...
# 支持向量机分类器
def train_svm(file_path=None,data=None, kernel='linear', test_size=0.2,        random_state=42,**kwargs
              ):
    """
    训练支持向量机（SVM）分类器。
    """
...
# 训练 AdaBoost 分类器
def train_adaboost(file_path=None, data=None, n_estimators=50, learning_rate=1.0, algorithm='SAMME', test_size=0.2, random_state=42,**kwargs
                   ):
    """
    训练 AdaBoost 分类器。
...
# 逻辑回归
def train_logistic_regression(file_path=None, data=None, penalty='l2', C=1.0,solver='lbfgs', max_iter=100, test_size=0.2, random_state=42,**kwargs
                              ):
    """
    训练逻辑回归分类器。
...
# 训练 K 近邻（KNN）分类器
def train_kneighbors(file_path=None, data=None, n_neighbors=5, weights='uniform', algorithm='auto', leaf_size=30, p=2, metric='minkowski', test_size=0.2, random_state=42,**kwargs
                     ):
    """
    训练 K 近邻（KNN）分类器。
...
# 训练随机森林分类器
def train_random_forest(file_path=None, data=None, n_estimators=100, criterion='gini', max_depth=None, min_samples_split=2, min_samples_leaf=1, max_features='sqrt', bootstrap=True, test_size=0.2, random_state=42,**kwargs
                        ):
    """
    训练随机森林分类器。
